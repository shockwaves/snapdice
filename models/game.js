game = {
    ...
    profile: function() {
        ...
        if (player.data.length) {
            $('#profile .name').val(player.name()).focus();
    ...
    drawing: function() {
        ...
        if(player.data[0].chance)
            $('#drawing [data-rel=face][value=0]').html(player.data[0].chance).show();
        else
            $('#drawing [data-rel=chance][value=0]').show();
        
        $('#drawing [data-rel=name][value=1]').html(player.second()).show();
        if(player.data[1].chance)
            $('#drawing [data-rel=face][value=1]').html(player.data[1].chance).show();
        else
            $('#drawing [data-rel=chance][value=1]').show();               
    },
    chance: function(_button) {
        _button.hide();
        var id = _button.attr('value');
              
        if(player.data[id].chance) {
            var value = player.data[id].chance;
        } else {
        ...
        if (id == 1) {
            if (player.data[0].chance == player.data[1].chance) {
                app.achievement++;
                player.data[0].chance = player.data[1].chance = 0;
                button('drawing').show();
            }
            else {
